...
\maketitle
これは小木曽啓示『代数曲線論』の5章を読み勉強した内容をノートにしたものである. 私は今野一宏『リーマン面と代数曲線』にてRiemann-Rochの定理を扱っているため, その知識と合流することを目的としたノートとなっている.
...
\begin{rem}
    ...
    \[\begin{tikzcd}
        ...
        \arrow["{\tau_i}", from=1-1, to=1-3]
    \end{tikzcd}\]
    を可換にし, \(U_{ij} \neq \emptyset\) なら, 同相写像 \(\tau_i \circ \tau_j^{-1} \colon \mathbb{C}^r \times U_{ij} \to \mathbb{C}^r \times U_{ij}\) は, ある \(n\times n\) 行列 \(g \in \textrm{GL}(n;\mathcal{O}_X(U_{ij}))\) によって, \(\tau_i \circ \tau_j^{-1} (l,z) = (g(z)l,z)\) (\((l,z) \in \mathbb{C}^r \times U_{ij}\)) と表されることを正則ベクトル束の定義としてもよい. そのときは, 正則ベクトル束の正則構造は \(\{\tau_i\}_{i\in I}\) で与えられる.
\end{rem}
